...
var addToRequestBuffer = require('./utils').addToRequestBuffer;
...
var Client = function(servers, options) {
  ...
  this.serverMap = {};
  for (var serverIndex in servers) {
    var server = servers[serverIndex];
    this.serverMap[server.host] = server;
  }

  this.hashring = new HashRing(
    Object.keys(this.serverMap),
    this.options.hashringAlgorithm || 'md5',
    this.options.hashringOptions || {
...
// * `hashringAlgorithm` algorithm to use for consistent hash, defaults to md5,
//                        must be something that the node crypto library supports
// * `hashringOptions` options for consistent hashing, see https://www.npmjs.com/package/hashring#options
...
Client.prototype.server = function(key) {
  return this.getServerByIndex(this.serverIndex(key));
};

Client.prototype.serverIndex = function(key) {
  var total = Object.keys(this.serverMap).length;
  if (total === 1) {
    return Object.keys(this.serverMap)[0];
  }

  return this.hashring.get(key);
};

Client.prototype.getServerByIndex = function(index) {
  return this.serverMap[index];
};
...
Client.prototype._buildGetMultiRequest = function(keys) {
  ...
  for (keyIdx in keys) {
    var key = keys[keyIdx];
    bytesWritten += addToRequestBuffer(constants.OP_GETKQ, key, '', '', this.seq, request, bytesWritten);
  }

  bytesWritten += addToRequestBuffer(
...
Client.prototype.getMulti = function(keys, callback) {
  ...
  var serverIndexToKeys = {};
  keys.forEach(function(key)  {
    var serverIndex = self.serverIndex(key);
    if (!serverIndexToKeys[serverIndex]) {
      serverIndexToKeys[serverIndex] = [];
    }
    serverIndexToKeys[serverIndex].push(key);
  });

  var outstandingCalls = Object.keys(serverIndexToKeys).length;
  var recordMap = {};
  var hadError = false;
  ...
  for (var serverIndex in serverIndexToKeys) {
    var server = this.getServerByIndex(serverIndex);
    var serverKeys = serverIndexToKeys[serverIndex];

    this._getMultiToServer(server, serverKeys, latchCallback);
  }
};
...
Client.prototype.performOnServer = function(server, request, seq, callback, retries) {
  ...
  var errorHandler = function(error) {
    if (--retries > 0) {
      ...
      logger.log('MemJS: Server <' + server.host + ':' + server.port +
