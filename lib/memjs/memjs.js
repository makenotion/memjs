...
Client.prototype.get = function(key, callback) {
  ...
  this.incrSeq();
  var request = makeRequestBuffer(constants.OP_SET, key, '', '', this.seq);
...
Client.prototype.perform = function(key, request, seq, callback, retries) {
  ...
  return this._performOnServer(server, request, seq, callback, retries);
};

Client.prototype._performOnServer = function(server, request, seq, callback, retries) {
  var _this = this;
  ...
  var errorHandler = function(error) {
    if (--retries > 0) {
      ...
      setTimeout(function() {
        _this._performOnServer(server, request, seq, callback, retries);
      }, 1000 * retry_delay);
    } else {
      ...
      if (failover) {
        server.wakeupAt = Date.now() + failoverTime * 1000;
        _this._performOnServer(server, request, seq, callback, origRetries);
      } else {
