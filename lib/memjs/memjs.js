...
var addToRequestBuffer = utils.addToRequestBuffer;
var makeRequestBuffer = utils.makeRequestBuffer;
...
var Client = function(servers, options) {
  ...
  this.hashring = new HashRing(
    servers.map(function (server) { return server.host; }),
...
Client.prototype.server = function(key) {
  ...
  var idx = (total > 1) ? this.hashring.get(key) : 0;
...
// _flags_ is a `Buffer`.
Client.prototype.multiGet = function(keys, callback) {
  var self = this;
  if(callback === undefined) {
    return promisify(function(callback) {
      self.multi_get(keys, function(err, values, flags) {
        callback(err, {values: values, flags: flags});
      });
    });
  }

  function sum(numbers) { return numbers.reduce(function (a, b) {return a + b;} , 0); }

  var request = Buffer.alloc(sum(keys.map(function(key) { return key.length + 24;})));
  request.fill();

  keys.forEach(function(key) {
    addToRequestBuffer(constants.OP_GETKQ, key, '', '', this.seq, request);
  });

  var logger = this.options.logger;
  this.incrSeq();
  // TODO: batch by key/server uhuh, send to the right server, etc
  this.performOnServer(this.servers[0], request, this.seq, function(err, response) {
    if (err) {
      if (callback) { callback(err, null, null); }
      return;
    }
...
Client.prototype.getPoolIndex = function(key) {
  if (this.servers.length === 1) {
    return 0;
  }
  var rawHost = this.hashring.get(key);
  var idx = this.servers[rawHost];
  return idx;
};
