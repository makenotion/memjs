...
var Client = function(servers, options) {
  ...
  var hosts = servers.map(function (server) {
    return server.host;
  });
...
Client.prototype.serverIndex = function(key) {
  ...
  return (total > 1) ? this.hashring.get(key) : 0;
};
...
Client.prototype.getMultiToServer = function (serv, keys, callback) {
  var self = this;
...
//
// _values_ is an array the length of keys, each one being a `Buffer` or `undefined`
// _flags_ is a `Buffer`.
Client.prototype.getMulti = function(keys, callback) {
  ...
  var serverIndexToKeys = _.groupBy(keys, function(key) {
    return self.serverIndex(key);
  });
  ...
  function latchCallback(err, values, flags) {
    console.log('errF', err, values, flags, hadError);
    if (hadError) {
      return;
    }
  ...
  for (var servIndex in serverIndexToKeys) {
    var serv = this.getServerByIndex(servIndex);
    var serverKeys = serverIndexToKeys[servIndex];
    this.getMultiToServer(serv, serverKeys, latchCallback);
  }
};
